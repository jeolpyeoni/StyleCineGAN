...
parser.add_argument('--real_dataset_path', type=str, default='/data/LHQ', help='dataset path')
...
parser.add_argument('--stylegan_model_path', type=str, default='/source/kseo/stylecinegan/stylegan2-pytorch/sg2-lhq-1024-baseline.pt', help='pretrained stylegan2 model')
parser.add_argument('--w_mean_path', type=str, default='/source/kseo/stylecinegan/stylegan2-pytorch/sg2-lhq-1024-baseline-mean.pt', help='pretrained stylegan2 model')
...
for n_epoch in tqdm(range(epoch_0, epochs)):
    ...
    with torch.no_grad():
        trainer.enc.eval()
        files = glob.glob('./test/personal/*.png')
        for f in files:
            name = os.path.basename(f).split('.')[0]
            image_A = img_to_tensor(Image.open(f)).unsqueeze(0).to(device)
            output = trainer.test(img=image_A)
            out_img = torch.cat(output, 3)
            utils.save_image(clip_img(out_img[:1]), log_dir + 'validation/' + 'epoch_' +str(n_epoch+1) + '_' + name + '.jpg')
