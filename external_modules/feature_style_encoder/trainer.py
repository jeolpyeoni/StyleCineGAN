...
class Trainer(nn.Module):
    def __init__(self, config, opts):
        ...
        self.enc = fs_encoder_v2(n_styles=self.n_styles, \
                                 opts=opts, 
    ...
    def initialize(self, stylegan_model_path, arcface_model_path, parsing_model_path, w_mean_path):
        ...
        try:
            ...
            self.StyleGAN.to(self.device)
            with torch.no_grad():
                # mean_path = os.path.basename(stylegan_model_path).split(".pt")[0] + "-mean.pt"
                # mean_path = os.path.basename(stylegan_model_path).split(".pt")[0] + "-mean.pt"
                self.dlatent_avg = torch.load(w_mean_path).to(self.device)
                # self.dlatent_avg = self.StyleGAN.mean_latent(10_000).to(self.device)
    ...
    def get_image(self, w=None, img=None, noise=None, zero_noise_input=True, training_mode=True):
        ...
        if 'use_fs_encoder' in self.config and self.config['use_fs_encoder']:
            ...
            # features = [None]*(k-1) + [fea] + [None]*(18-k)
        else:
        ...
        # print("*"*10)
        # for i, f in enumerate(fea_recon):
            # print(f'{i}', f.size())
        # print("-"*5)
        # print()
        # print(k, fea.shape)
    ...
    def compute_loss_stylegan2(self, w=None, img=None, noise=None, real_img=None):
        ...
        if 'use_fs_encoder' in self.config and self.config['use_fs_encoder']:
            ...
            # features = [None]*(k-1) + [fea_1] + [None]*(18-k)
