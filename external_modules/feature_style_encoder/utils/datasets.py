...
class MyDataSet(data.Dataset):
    def __init__(self, image_dir=None, label_dir=None, output_size=(256, 256), noise_in=None, training_set=True, video_data=False, train_split=0.9):
        ...
        if "LHQ" in self.image_dir:
            if image_dir is not None:
                tmp = os.path.join(self.image_dir, "*0000")
                image_list = []
                image_list += glob.glob(os.path.join(tmp, '*.jpg'))
                image_list += glob.glob(os.path.join(tmp, '*.png'))

                image_list = [f.split(self.image_dir +'/')[-1] for f in image_list]
    ...
    def __getitem__(self, idx):
        ...
        if self.image_dir is not None:
            ...
            # print(img_name, self.image_dir)
            # print(os.path.join(self.image_dir, self.image_list[idx]))
